...
class VaStepVendorFile(VaStep):
    def __init__(self, parent_wizard, previous_step, *args, **kwargs):
        ...
        self.kymera_dir = os.path.join(os.path.dirname(self.workspace), '..', '..', '..',
                                       'audio', self.chip_type, 'kalimba', 'kymera')
    ...
    def add_files_to_ro_project(self):
        ...
        dkcs_file = os.path.splitext(os.path.normpath(os.path.join(os.path.dirname(self.capability_project), elf_file)))[0] + ".dkcs"
        patch = '<project><file path="{}"/></project>'.format(os.path.relpath(dkcs_file, os.path.dirname(self.ro_fs_project)))
