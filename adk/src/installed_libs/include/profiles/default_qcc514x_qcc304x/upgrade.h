...
#define UPGRADE_LIBRARY_CONTEXT_OFFSET          2   /*!< Offset with that key to area for upgrade library */
...
typedef enum
{
    ...
    upgrade_ops_handle_validation_state_change,
    ...
    upgrade_ops_set_validation_status
} upgrade_ops_t;
...
/*!
    @brief UpgradeSmHandleValidationStateChange

    Handle Upgrade Validating state for the next stage of DFU

    @param was_peer_dfu_in_progress Used to check if peer dfu is supported
           and was in progress earlier
    @return None
*/
