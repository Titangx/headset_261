...
typedef struct
{
    ...
    hfp_link_priority_t priority;           /*!< Priority of the link - used for multiple hfp connections */
    bdaddr bd_addr;                     /*!< Address of AG */
...
/*! \brief Initiate HFP profile connection with device
    ...
    \param  client_task The task to notfiy when the profile is connected
    \param  bd_addr Bluetooth address of the HFP AG to connect to
*/
...
/*! \brief Disconnect HFP profile from a device
    ...
    \param  client_task The task to notfiy when the profile is disconnected
