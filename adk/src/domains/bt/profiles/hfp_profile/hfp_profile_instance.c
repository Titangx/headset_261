    ...
    switch (appHfpGetState(instance))
    {
        case HFP_STATE_CONNECTED_IDLE:
        {
            ...
            if (link != hfp_invalid_link)
            {
                ...
                {
                    /* Send button press */
                    HfpHsButtonPressRequest(link);
                }
                else
                {
                    HfpDialLastNumberRequest(link);
                }
            }
        }
        return;
    ...
    switch (appHfpGetState(instance))
    {
        case HFP_STATE_CONNECTED_IDLE:
        {
            ...
            if (link != hfp_invalid_link)
            {
                ...
                {
                    HfpVoiceRecognitionEnableRequest(link, instance->bitfields.voice_recognition_request = TRUE);
    ...
    switch (appHfpGetState(instance))
    {
        case HFP_STATE_CONNECTED_IDLE:
        {
            ...
            if (link != hfp_invalid_link)
            {
                ...
                {
                   HfpVoiceRecognitionEnableRequest(link, instance->bitfields.voice_recognition_request = FALSE);
                }
            }
        }
        return;
...
static void appHfpHandleInternalNumberDialRequest(HFP_INTERNAL_NUMBER_DIAL_REQ_T * message)
{
    ...
    switch (appHfpGetState(message->instance))
    {
        case HFP_STATE_CONNECTED_IDLE:
        {
            voice_source_t source = HfpProfileInstance_GetVoiceSourceForInstance(message->instance);
            hfp_link_priority link = HfpProfile_GetHfpLinkPrioForVoiceSource(source);
            if (link != hfp_invalid_link)
            {
                ...
                {
                    HfpDialNumberRequest(link, message->length, message->number);
                }
            }
        }
        return;
    ...
    switch (state)
    {
        case HFP_STATE_CONNECTED_INCOMING:
        {
            ...
            if (link != hfp_invalid_link)
            {
                ...
                {
                    HfpCallAnswerRequest(link, TRUE);
                }
            }
            else
            {
                DEBUG_LOG_ERROR("appHfpHandleInternalHfpCallAcceptRequest link invalid for source");
            }
        }
        return;
    ...
    switch (appHfpGetState(instance))
    {
        case HFP_STATE_CONNECTED_INCOMING:
        {
            ...
            if (link != hfp_invalid_link)
            {
                if (instance->profile == hfp_headset_profile)
                {
                    Telephony_NotifyError(HfpProfile_GetVoiceSourceForHfpLinkPrio(link));
                }
                else
                {
                    HfpCallAnswerRequest(link, FALSE);
                }
            }
        }
        return;
...
static void appHfpHandleInternalHfpTransferRequest(const HFP_INTERNAL_HFP_TRANSFER_REQ_T *req)
{
    ...
    switch (appHfpGetState(instance))
    {
        ...
        case HFP_STATE_CONNECTED_OUTGOING:
        {
            hfp_link_priority link = HfpProfile_GetHfpLinkPrioForVoiceSource(req->source);
            if (link != hfp_invalid_link)
            {
                HfpAudioTransferRequest(link, hfpProfile_GetVoiceSourceHfpDirection(req->direction),
...
static device_t hfpProfileInstance_FindDeviceFromVoiceSource(voice_source_t source)
{
...
hfpInstanceTaskData * HfpProfileInstance_Create(const bdaddr *bd_addr, bool allocate_source)
{
    ...
    if (appDeviceIsHandset(bd_addr))
    {
        if(allocate_source)
        {
            ...
            HfpProfile_UpdateMappingOnConnection(new_source);
    ...
    //HfpProfile_SendStatusMessage(HFP_CREATE_IND, NULL, 0);
...
void HfpProfileInstance_Destroy(hfpInstanceTaskData *instance)
{
    ...
    if (HfpProfile_IsDisconnected(instance) /*&& !HfpProfile_IsLockPending()*/)
    {
        ...
        HfpProfile_UpdateMappingOnDisconnection(source);
        ...
        //HfpProfile_SendStatusMessage(AV_DESTROY_IND, NULL, 0);
    }
    else
    {
