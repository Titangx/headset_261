...
static volume_t hfpProfile_GetVolume(voice_source_t source)
{
    ...
    if(instance != NULL)
    {
        ...
        Device_GetPropertyU8(device, device_property_hfp_volume, (uint8 *)&volume.value);
    }
    else if (MirrorProfile_IsConnected())
    {
        ...
        if (device != NULL && Device_IsPropertySet(device, device_property_hfp_volume))
        {
            Device_GetPropertyU8(device, device_property_hfp_volume, (uint8 *)&volume.value);
        }
        else
        {
            volume.value = HfpProfile_GetDefaultVolume();
        }
    }
...
static void hfpProfile_SetVolume(voice_source_t source, volume_t volume)
{
    ...
    if(instance != NULL)
    {
        ...
        Device_SetPropertyU8(device, device_property_hfp_volume, volume.value);
