 ...
 */
static void hfpProfile_SendSlcStatus(bool connected, hfp_link_priority priority, const bdaddr* bd_addr)
{
    Task next_client = 0;
    ...
    switch (state)
    {
        ...
        case HFP_STATE_CONNECTING_REMOTE:
        {
            ...
            if (cfm->status == hfp_connect_success)
            {
                ...
                appHfpSetState(instance, hfp_call_state_table[instance->bitfields.call_state]);

                /* inform clients */
                hfpProfile_SendSlcStatus(TRUE, cfm->priority, &cfm->bd_addr);
...
static void hfpProfile_HandleHfpSlcDisconnectInd(const HFP_SLC_DISCONNECT_IND_T *ind)
{
    ...
    switch (state)
    {
        ...
        case HFP_STATE_CONNECTED_ACTIVE:
        {
            ...
            /* Inform clients */
            hfpProfile_SendSlcStatus(FALSE, ind->priority, HfpProfile_GetHandsetBdAddr(instance));
...
bool HfpProfile_Init(Task init_task)
{
    ...
    HfpProfile_InitHfpLinkPrioMapping();
...
{
    uint8 volume = HfpProfile_GetDefaultVolume();
    device_t device = DeviceList_GetFirstDeviceWithPropertyValue(device_property_hfp_instance,
                                                                 &instance, sizeof(hfpInstanceTaskData *));
    PanicNull(device);
    Device_GetPropertyU8(device, device_property_hfp_volume, &volume);
    return volume;
}
...
uint8 HfpProfile_GetDefaultVolume(void)
{
    return HFP_SPEAKER_GAIN;
}
