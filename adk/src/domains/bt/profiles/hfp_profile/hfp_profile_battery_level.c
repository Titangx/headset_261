...
static const batteryRegistrationForm hfp_battery_form = {
        .task = (Task)&hfp_profile_task_data.task,
        .hysteresis = 1,
        .representation = battery_level_repres_percent
};
#endif
...
void HfpProfile_BatteryLevelInit(void)
{
    ...
    use_local_only = TRUE;
#else
    appBatteryRegister((batteryRegistrationForm *)&hfp_battery_form);
#endif
...
inline static uint8 hfpProfile_GetBatteryLevel(void)
{
    ...
    last_biev_battery_level = appBatteryConvertLevelToPercentage(lower_battery_level);
#else
    last_biev_battery_level =  appBatteryGetPercent();
#endif
...
inline static void hfpProfile_SendBievCommand(void)
{
    ...
    for_all_hfp_instances(instance, &iterator)
    {
        DEBUG_LOG_VERBOSE("hfpProfile_SendBievCommand instance %d lap 0x%x state enum:hfpState:%d hf_ind enum:hfp_indicators_assigned_id:%d",
                iterator.index, instance->ag_bd_addr.lap, appHfpGetState(instance), instance->bitfields.hf_indicator_assigned_num);
...
void HfpProfile_HandleBatteryMessages(MessageId id, Message message)
{
    ...
    switch(id)
    {
        ...
        case MESSAGE_BATTERY_LEVEL_UPDATE_PERCENT:
            DEBUG_LOG_VERBOSE("HfpProfile_HandleBatteryMessages MESSAGE_BATTERY_LEVEL_UPDATE_PERCENT");
            break;
#endif
        default:
