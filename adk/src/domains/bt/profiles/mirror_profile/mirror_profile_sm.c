...
void MirrorProfile_CompleteVoiceSourceConnection(void)
{
...
static void mirrorProfile_EnterDisconnected(void)
{
    ...
    MessageSend(MirrorProfile_GetTask(), MIRROR_INTERVAL_KICK_TARGET_STATE, NULL);
}
...
static void mirrorProfile_EnterAclConnected(void)
{
    ...
    appDeviceUpdateMruDevice(MirrorProfile_GetMirroredDeviceAddress());
    ...
    if (MirrorProfile_IsPrimary())
    {
        ...
        {
            /* Required to reread the parameters if switching mirrored handsets */
...
static void mirrorProfile_EnterA2dpConnecting(void)
{
    ...
    else
    {
        ...
        {
            /* Audio will be started when the stream context message is received
            from the secondary */
        }
    }
}
