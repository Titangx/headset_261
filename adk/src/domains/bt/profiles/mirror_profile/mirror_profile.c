...
/* This requires new support in BTSS and appsP0 */
            ...
            else
            {
                ...
                if (MirrorProfile_IsAclConnected())
                {
                    ...
                    else if (MirrorProfile_IsA2dpMirroringEnabled())
                    {
...
{
    if (1 == appDeviceNumOfHandsetsConnected())
    {
...
{
    audio_source_t mirrored_source = MirrorProfile_GetAudioSource();
    bool reply_immediately = !MirrorProfile_IsAudioSyncL2capConnected();

    if(mirrored_source == ind->source_id)
    {
        if(MirrorProfile_StoreAudioSourceParameters(ind->source_id))
        {
            MIRROR_LOG("MirrorProfile_HandleAudioSyncPrepareInd");
    ...
    {
        mirrorProfile_SendAudioSyncPrepareRes(ind->source_id, audio_sync_not_required);
    }
}
...
static void mirrorProfile_MessageHandler(Task task, MessageId id, Message message)
{
    ...
    case MIRROR_INTERVAL_KICK_TARGET_STATE:
        MirrorProfile_SetTargetStateFromProfileState();
        break;
...
mirror_profile_a2dp_start_mode_t MirrorProfile_GetA2dpStartMode(void)
{
    ...
    /* Q2Q status is only known when a2dp is connected, so we need to rely on the kymera variable before this happens.
     * This can happen when only a single earbud is active */
    bool is_q2q = MirrorProfile_IsA2dpConnected() ? MirrorProfile_IsQ2Q() : KymeraGetTaskData()->q2q_mode;
    bool sync_start;   
    ...
    if (MirrorProfile_IsPrimary())
    {
        if (sync_start)
        {
            ...
            {
                mode = is_q2q ? MIRROR_PROFILE_A2DP_START_Q2Q_MODE : MIRROR_PROFILE_A2DP_START_PRIMARY_SYNCHRONISED;
            }
        }
        else
        {
            mode = is_q2q ? MIRROR_PROFILE_A2DP_START_Q2Q_MODE :
...
void MirrorProfile_DisableMirrorEsco(void)
{
    ...
    /* If eSCO mirroring is disabled before the voice source is connected there
       will be no trigger to complete the connection. Therefore this must be
...
void MirrorProfile_HandleHfpAudioConnectConfirmation(voice_source_t source)
{
    ...
    if (MirrorProfile_IsAclConnected() &&
        ...
        MirrorProfile_IsVoiceSourceSupported(source))
    {
