...
{
    uint16 supported_features = aghfp_inband_ring |
                                aghfp_incoming_call_reject |
                                aghfp_codec_negotiation;

    AghfpInit(&aghfp_profile_task_data.task,
...
static void aghfpProfile_HandleSlcDisconnectInd(AGHFP_SLC_DISCONNECT_IND_T *message)
{
    ...
    DEBUG_LOG("aghfpProfile_HandleSlcDisconnectInd(%p) enum:aghfp_disconnect_status:%d", instance, message->status);
...
{
    DEBUG_LOG_INFO("AghfpProfile_Connect");
...
{
    DEBUG_LOG_INFO("AghfpProfile_Disconnect");

    aghfpInstanceTaskData* instance = AghfpProfileInstance_GetInstanceForBdaddr(bd_addr);

    if (!instance)
    {
        DEBUG_LOG_INFO("AghfpProfile_DisconnectWithBdAddr - No instance found");
        return;
    }
...
{
    DEBUG_LOG_INFO("AghfpProfile_Init");
    UNUSED(init_task);
...
{
    DEBUG_LOG_INFO("AghfpProfile_CallIncomingInd");
...
{
    DEBUG_LOG_INFO("AghfpProfile_CallOutgoingInd");
...
{
    DEBUG_LOG_INFO("AghfpProfile_OutgoingCallAnswered");
