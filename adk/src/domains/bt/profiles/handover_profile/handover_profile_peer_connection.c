...
{
    uint16_t status = ind->status;
    Sink sink = ind->sink;
    handover_profile_task_data_t *ho_inst = Handover_GetTaskData();

    DEBUG_LOG("HandoverProfile_HandleL2capDisconnectInd, status %u", status);
    ...
    if (!handoverProfile_IsDisconnected(ho_inst))
    {
        ...
        if (sink == ho_inst->link_sink)
        {
            /* Inform clients if link loss and we initiated the original connection */
            if (status == l2cap_disconnect_link_loss && !BdaddrIsZero(&ho_inst->peer_addr))
            {
