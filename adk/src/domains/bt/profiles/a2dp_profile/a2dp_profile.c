...
static void appA2dpStopAudio(avInstanceTaskData *theInst)
{
    ...
    AvSendAudioConnectedStatusMessage(theInst, FALSE);
                                                      ...
                                                      const AV_INTERNAL_A2DP_CODEC_RECONFIG_IND_T *ind)
{
    ...
    switch (local_state)
    {
        case A2DP_STATE_CONNECTED_MEDIA_SUSPENDED:
        {
            ...
            {
                DEBUG_LOG_ERROR("appA2dpHandleInternalA2dpCodecReconfigInd:A2dpCodecGetSettings failed %p %p",
                          src_codec_settings, snk_codec_settings);
            }
...
void appA2dpInstanceInit(avInstanceTaskData *theAv, uint8 suspend_state)
{
    ...
    theAv->a2dp.source_state = source_state_disconnected;
...
source_status_t A2dpProfile_SetSourceState(avInstanceTaskData *av_instance, source_state_t state)
{
    ...
    if(state == source_state_disconnected)
    {
