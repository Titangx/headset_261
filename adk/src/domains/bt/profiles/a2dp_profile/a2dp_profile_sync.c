                                                     ...
                                                     const AUDIO_SYNC_PREPARE_RES_T *res)
{
    ...
    if (((res->sync_id + 1) & 0xff) != theInst->a2dp.sync_counter)
    {
        if(!a2dpIsSyncFlagSet(theInst, A2DP_SYNC_PREPARE_RESPONSE_PENDING) && !a2dpIsSyncFlagSet(theInst, A2DP_SYNC_MEDIA_START_PENDING))
        {
            ...
            A2DP_SYNC_LOG("appA2dpSyncHandleA2dpSyncPrepareResponse(%p) late state(0x%x) sync_id(%d) sync_count(%d) reason (%d)",
    ...
    A2DP_SYNC_LOG("appA2dpSyncHandleA2dpSyncPrepareResponse(%p) state(0x%x) sync_id(%d) reason (%d)",
    ...
    if(res->reason != audio_sync_timeout)
    {
        ...
        AvSendAudioConnectedStatusMessage(theInst, TRUE);
        theInst->a2dp.sync_counter++;
    }
                                                      ...
                                                      const AUDIO_SYNC_ACTIVATE_RES_T *res)
{
    ...
    DEBUG_LOG("appA2dpSyncHandleA2dpSyncActivateResponse: sync_response_rcvd %d", theInst->a2dp.bitfields.sync_response_rcvd);
...
void a2dpProfileSync_SendPrepareIndication(avInstanceTaskData *av_instance)
{
    ...
    a2dpSetSyncFlag(av_instance, A2DP_SYNC_MEDIA_START_PENDING | A2DP_SYNC_PREPARE_RESPONSE_PENDING);
