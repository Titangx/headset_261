...
static void pairing_EnterDiscoverable(pairingTaskData *thePairing)
{
    ...
    MessageCancelAll(&thePairing->task, PAIRING_LE_PAIR_TIMEOUT_IND);
            ...
            if (cfm->status == auth_status_success)
            {
                ...
                if (cfm->bonded)
                {
                    ...
                    if(!expected_ble) /* Don't consider ble complete until CL_SM_BLE_SIMPLE_PAIRING_COMPLETE_IND */
                    {
                        ...
                        /* Allow ble connections only with paired handset after br/edr pairing. */
                        thePairing->ble_permission = pairingBleOnlyPairedHandsets;
                        MessageSendLater(&thePairing->task, PAIRING_LE_PAIR_TIMEOUT_IND, NULL,
...
static void pairing_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case PAIRING_LE_PAIR_TIMEOUT_IND:
