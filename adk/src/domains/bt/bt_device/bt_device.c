...
static void btDevice_PrintDeviceInfo(device_t device, void *data)
{
    ...
    {
        DEBUG_LOG("a2dp volume %d", volume);
    }

    if (Device_GetPropertyU8(device, device_property_hfp_volume, (void *)&volume))
    {
        DEBUG_LOG("hfp volume %d", volume);
    }
}
...
bool BtDevice_SetDefaultProperties(device_t device)
{
    ...
    if(!Device_SetPropertyU8(device, device_property_hfp_volume, HfpProfile_GetDefaultVolume()))
    {
        return FALSE;
    }
