...
{
    DEBUG_LOG_LEVEL_1("leAdvertisingManager_handleSetRandomAddressCfm enum:connection_lib_status:%d ADDR: %04x%02x%06lx", 
                        confirm->status,
...
{
    PanicFalse(leAdvertisingManager_CheckBlockingCondition(ADV_SETUP_BLOCK_NONE));
    
    leAdvertisingManager_SetBlockingCondition(ADV_SETUP_BLOCK_ADV_REGISTER_CFM);
...
{    
    ...
    {
        DEBUG_LOG_LEVEL_2("leAdvertisingManager_HandleExtendedSetScanResponseDataCfm Info, adv_task_data->blockingCondition is %x cfm->status is %x", leAdvertisingManager_GetBlockingCondition());
...
void leAdvertisingManager_HandleExtendedSetAdvertisingParamCfm(const CL_DM_BLE_SET_EXT_ADV_PARAMS_CFM_T *cfm)
{
    ...
    {            
        DEBUG_LOG_LEVEL_2("leAdvertisingManager_HandleExtendedSetAdvertisingParamCfm Info, adv_task_data->blockingCondition is %x cfm->status is %x", leAdvertisingManager_GetBlockingCondition());
...
void leAdvertisingManager_HandleExtendedAdvertisingRegisterCfm(const CL_DM_BLE_EXT_ADV_REGISTER_APP_ADV_SET_CFM_T *cfm)
{
    ...
    if (leAdvertisingManager_CheckBlockingCondition(ADV_SETUP_BLOCK_ADV_REGISTER_CFM))
    {
        leAdvertisingManager_SetBlockingCondition(ADV_SETUP_BLOCK_NONE);
