...
static profile_manager_disconnected_ind_reason_t profileManager_ConvertA2dpDisconnectReason(avA2dpDisconnectReason a2dp_reason)
{
    profile_manager_disconnected_ind_reason_t reason;
    switch (a2dp_reason)
    {
...
{
    DEBUG_LOG("profileManager_HandleProfileStatusChange device %p profile %d type %u status %d",
...
static void profileManager_HandleConnectedProfileInd(const bdaddr* bd_addr, uint32 profile, profile_request_status_t status)
{
    ...
    DEBUG_LOG("profileManager_HandleConnectedProfileInd bdaddr nap=%x uap=%x lap=%x, profile=%d, status=%d",
...
static void profileManager_HandleMessage(Task task, MessageId id, Message message)
{
    ...
    DEBUG_LOG("profileManager_HandleMessage MESSAGE:0x%04X", id);
...
bool ProfileManager_Init(Task init_task)
{
    ...
    HfpProfile_RegisterStatusClient(&pm->task);
...
void ProfileManager_GenericDisconnectedInd(profile_t profile, const bdaddr *bd_addr, uint8 reason)
{
