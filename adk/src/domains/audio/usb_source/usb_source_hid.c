...
{
    if (UsbSource_IsAudioSupported() &&
            usb_source_hid_interface &&
            usb_source_hid_interface->send_event)
    {
...
void UsbSource_IncomingCallAccept(voice_source_t source)
{
    ...
    {
        /* This is a basic implementation which may not work with all hosts. */
        UsbSource_SendEvent(USB_SOURCE_PLAY_PAUSE);
    }
}
