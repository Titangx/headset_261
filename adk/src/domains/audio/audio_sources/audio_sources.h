...
/*! \brief Audio Sources UI Provider contexts
    \note This enumerated type must be maintained in order of ascending priority.
          An Audio Source which has been disconnected has the lowest priority in
          audio routing, whereas an audio source that is streaming has the highest
          priority. */
typedef enum
{
    context_audio_disconnected,     /*! Disconnected */
    context_audio_connected,        /*! Connected but not receiving an audio stream */
    context_audio_is_streaming,     /*! Receiving an audio stream with undefined content (tones, music, video, etc) */
    context_audio_is_playing,       /*! Receiving an audio stream with playing content (music or video) */
    context_audio_is_resumed,       /*! Receiving an audio stream that was explicitly resumed */
    context_audio_is_va_response,   /*! Receiving an audio stream that is a Voice Assistant response */
    max_audio_contexts
