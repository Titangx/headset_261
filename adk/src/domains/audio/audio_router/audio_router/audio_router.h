...
typedef struct
{
    ...
    void (*remove_source)(generic_source_t source);
    bool (*is_device_in_use)(device_t device);
    void (*update)(void);
} audio_router_t;

/*! Interface for indications regarding voice source routing state.
*/
typedef struct
{
    /*! To be called immediately before voice source routed */
    void (*VoiceRoutingInd)(void);
    /*! To be called immediately after voice source unrouted */
    void (*VoiceUnroutedInd)(void);
} audio_router_voice_routing_ind_t;
...
/*! \brief Register to receive indications regarding voice routing
