...
{
    if((AncStateManager_GetCurrentMode() == ADAPTIVE_ANC_MODE) && AncStateManager_IsEnabled())
    {
...
static Source ancStateManager_GetHeadMicSource(void)
{
    Source head_mic_source = NULL;
    microphone_number_t head_mic_number = MAX_SUPPORTED_MICROPHONES;
    ...
    if (ancConfigManagerGetReadOnlyConfig(ANC_READONLY_CONFIG_BLK_ID, (const void **)&read_data))
    {
        ...
        /*Microphone number signifies the connection sequence to aec_ref.
          In case head mic is not connected before the non head microphone
          the aec_ref will block the microphone data flow.*/

        if(feedForwardLeftMic)
        {
           head_mic_number = MIN(head_mic_number,feedForwardLeftMic);
        }

        if(feedBackLeftMic)
        {
            head_mic_number = MIN(head_mic_number,feedBackLeftMic);
        }

        if(feedForwardRightMic)
        {
            head_mic_number = MIN(head_mic_number,feedForwardRightMic);
        }

        if(feedBackRightMic)
        {
            head_mic_number = MIN(head_mic_number,feedBackRightMic);
        }

        DEBUG_LOG("ancStateManager_GetHeadMicSource: %d microphone selected as head microphone.",head_mic_number);
        head_mic_source = Microphones_GetMicrophoneSource(head_mic_number);

    }
    else
    {
        DEBUG_LOG_ALWAYS("ancStateManager_GetHeadMicSource: ANC mics are not configured.");
        Panic();
    }

    return head_mic_source;
}

static void ancStateManager_SynchronizeAncMics(void)
{
    Source head_mic_source = ancStateManager_GetHeadMicSource();
    if(head_mic_source && anc_data.mic_src_ff_left)
    {
        SourceSynchronise(head_mic_source, anc_data.mic_src_ff_left);
    }

    if(head_mic_source && anc_data.mic_src_fb_left)
    {
        SourceSynchronise(head_mic_source, anc_data.mic_src_fb_left);
    }

    if(head_mic_source && anc_data.mic_src_ff_right)
    {
        SourceSynchronise(head_mic_source, anc_data.mic_src_ff_right);
    }

    if(head_mic_source && anc_data.mic_src_fb_right)
    {
        SourceSynchronise(head_mic_source, anc_data.mic_src_fb_right);
    }
}
#endif
...
#ifndef INCLUDE_MIC_CONCURRENCY
        ancStateManager_SynchronizeAncMics();
#endif
