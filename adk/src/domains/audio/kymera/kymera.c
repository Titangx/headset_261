...
static void appKymeraSetupForConcurrentCase(MessageId msg_id)
{
        ...
        switch (msg_id)
        {
            ...
            case KYMERA_INTERNAL_SCOFWD_RX_STOP:
            {
                ...
                KymeraAdaptiveAnc_ReconfigureAancOnScoStop();
                /* Doesnt matter if kymera is busy with anything else,we are no more in concurrent state */
                appKymeraSetConcurrentState(KYMERA_CON_STATE_NONE);
#endif
                       ...
                       uint8 wesco, int16 volume_in_db, uint8 pre_start_delay)
{
    ...
    const appKymeraScoChainInfo *info = appKymeraScoFindChain(appKymeraScoChainTable,
                                                              ...
                                                              mic_cfg);
    PanicFalse(!((mic_cfg == 3) && (mode == SCO_SWB)));    /* 3Mic SWB is not supported. No CAP_IDs */
    if (!info)
...
static void kymera_msg_handler(Task task, MessageId id, Message msg)
{
    ...
    switch (id)
    {
        ...
        case KYMERA_INTERNAL_USB_VOICE_START:
        {
            ...
            //KymeraUsbScoVoice_Start((KYMERA_INTERNAL_USB_VOICE_START_T *)msg);
        ...
        case KYMERA_INTERNAL_USER_EQ_SET_USER_GAINS:
        {
            ...
            free(m->gain);
            TaskList_MessageSendId(theKymera->listeners, KYMERA_NOTIFCATION_USER_EQ_BANDS_UPDATED);
        }
        break;
