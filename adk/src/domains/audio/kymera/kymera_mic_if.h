...
\brief  List of possible events that are sent to the users to inform about
        ...
        mic_event_connecting will be sent when a new user connects. Additional info about the reason for the disconnection is:
        - an extra mic required by the new user
        - a higher sample rate is required by the new user
        mic_event_disconnecting will be sent whe an existing user disconnects which results in a change in mic configuration.
*/
typedef enum
{
    mic_event_extra_mic = (1 << 0),
    mic_event_higher_sample_rate = (1 << 1),
    mic_event_connecting = (1 << 2),
    mic_event_disconnecting = (1 << 3)
} mic_event_t;
...
/*! \brief Attach a microphone to a speaker via the internal leakthrough path of AEC Ref.
    ...
    \param user name of requesting user
    \param mic_params connection infos like sample rate and microphone(s)
 */
