...
void appKymeraSetMainVolume(kymera_chain_handle_t chain, int16 volume_in_db)
{
    ...
    {
        OperatorsVolumeSetMainGain(volop, Kymera_VolDbToGain(volume_in_db));
    }
}
...
void appKymeraSetAuxVolume(kymera_chain_handle_t chain, int16 volume_in_db)
{
    ...
    if (GET_OP_FROM_CHAIN(volop, chain, OPR_VOLUME_CONTROL))
    {
        OperatorsVolumeSetAuxGain(volop, Kymera_VolDbToGain(volume_in_db));
    }
}
...
void appKymeraSetVolume(kymera_chain_handle_t chain, int16 volume_in_db)
{
    ...
    if (GET_OP_FROM_CHAIN(volop, chain, OPR_VOLUME_CONTROL))
    {
        OperatorsVolumeSetMainAndAuxGain(volop, Kymera_VolDbToGain(volume_in_db));
    }
}
...
uint32 appKymeraGetOptimalSpeakerInputRate(uint32 sample_rate)
{
  /*In legacy NB-SCO/AANC concurrency use case resampling by aec_ref is not required due to presence of additional resampler in sco chain.
    This requirement is valid only if INCLUDE_MIC_CONCURRENCY is not defined.*/

#if defined(ENABLE_ADAPTIVE_ANC) && !defined(INCLUDE_MIC_CONCURRENCY)
...
void Kymera_ConnectOutputSource(Source left, Source right, uint32 output_sample_rate)
{
    ...
    if(appConfigOutputIsStereo() || appKymeraIsParallelAncFilterEnabled())
    {
