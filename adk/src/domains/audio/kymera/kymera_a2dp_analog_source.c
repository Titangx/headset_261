...
static void kymeraA2dpAnalogSource_CreateInputChain(kymeraTaskData *theKymera, uint8 seid)
{
    ...
    switch (seid)
    {
        ...
        case AV_SEID_APTX_ADAPTIVE_SRC:
        {
            ...
            config = Kymera_GetChainConfigs()->chain_input_wired_aptx_encode_config;
            break;
        }
        default:
            Panic();
                                         ...
                                         const a2dp_codec_settings *codec_settings, uint32 min_latency, uint32 max_latency, uint32 target_latency)
{
    ...
    OperatorsConfigureTtpPassthrough(ttp_passthrough, TTP_LATENCY_IN_US(target_latency), codec_settings->rate, operator_data_format_pcm);
    ...
    switch (codec_settings->seid)
    {
        case AV_SEID_SBC_SRC:
        {
            ...
            sbc_encoder_params_t a;

            a.channel_mode = codec_settings->channel_mode;
            a.bitpool_size = codec_settings->codecData.bitpool;
            a.sample_rate = codec_settings->rate;
            a.number_of_subbands = SBC_SUBBANDS_8;
            a.number_of_blocks = SBC_BLOCK_LENGTH_16;
            a.allocation_method = sbc_encoder_allocation_method_loudness;

            OperatorsSbcEncoderSetEncodingParams(sbc_encoder, &a);
        }
        break;
