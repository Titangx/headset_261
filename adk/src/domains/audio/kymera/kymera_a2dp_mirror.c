...
static void appKymeraStartChains(kymeraTaskData *theKymera)
{
    ...
    case KYMERA_AUDIO_SYNC_START_SECONDARY_SYNC_UNMUTE:
    {
        ...
        appKymeraSourceSyncSetMonoRouteGain(output, theKymera->output_rate, 0, VOLUME_MUTE_IN_DB);
...
void appKymeraA2dpStartForwarding(const a2dp_codec_settings *codec_settings)
{
    ...
    DEBUG_LOG("appKymeraA2dpStartForwarding a2dp_start_mode enum:mirror_profile_a2dp_start_mode_t:%d", mode);
...
{
    kymeraTaskData *theKymera = KymeraGetTaskData();
    rtime_t now, sched_delay;

    now = VmGetTimerTime();
    if (rtime_gt(sync_time, now))
    {
        sched_delay = (rtime_sub(sync_time, now) + US_PER_MS) / US_PER_MS;
    }
    else
    {
        sched_delay = D_IMMEDIATE;
    }
...
void appKymeraA2dpHandleAudioSynchronisedInd(void)
{
    ...
    case KYMERA_AUDIO_SYNC_START_PRIMARY_SYNC_UNMUTE:
    {
        ...
        appKymeraSourceSyncSetMonoRouteGain(output, theKymera->output_rate,
            appConfigA2dpSyncUnmuteTransitionSamples(), 0);
...
void appKymeraA2dpHandleMessageMoreData(const MessageMoreData *mmd)
{
    ...
    if (mmd->source == theKymera->media_source)
    {
        ...
        if (boundary)
        {
            ...
            /* The secondary calls MirrorProfile_SendA2pdUnmuteTimeToPrimary() when it handles the
               audio sync stream indication (MESSAGE_SOURCE_AUDIO_SYNCHRONISED) but with Q2Q, there
               is no audio sync stream ind, and so there will be no MESSAGE_SOURCE_AUDIO_SYNCHRONISED
               to trigger the secondary to send the sync message.
               Another way of detecting on the secondary that audio has started streaming from the 
               handset is MESSAGE_MORE_DATA.
               If in Q2Q mode, secondary calls MirrorProfile_SendA2pdUnmuteTimeToPrimary() so it can
               schedule a unmute instant and send the unmute time to the primary. Then the primary can
               unmute at the same time.
            */
            if (   theKymera->q2q_mode 
                && (theKymera->sync_info.mode == KYMERA_AUDIO_SYNC_START_SECONDARY_SYNC_UNMUTE)
                )
