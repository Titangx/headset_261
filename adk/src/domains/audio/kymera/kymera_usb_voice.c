...
void KymeraUsbVoice_Start(KYMERA_INTERNAL_USB_VOICE_START_T *usb_voice)
{
    ...
    Source mic_src_1a = Kymera_GetMicrophoneSource(appConfigMicVoice(),
                        NULL,appKymeraGetOptimalMicSampleRate(usb_voice->mic_sample_rate),
                        high_priority_user);

    Source mic_src_1b = Kymera_GetMicrophoneSource((nr_mics > 1) ? appConfigMicExternal() : microphone_none,
                         mic_src_1a, appKymeraGetOptimalMicSampleRate(usb_voice->mic_sample_rate),
                         high_priority_user);

    DEBUG_LOG_VERBOSE("USB Voice: mic 1 and 2 are %x, %x", mic_src_1a, mic_src_1b);
    ...
    aec_config.mic_sample_rate = usb_voice->mic_sample_rate;
...
void KymeraUsbVoice_Stop(KYMERA_INTERNAL_USB_VOICE_STOP_T *usb_voice)
{
    ...
    Kymera_OutputDestroyingCustomChain(output_user_usb_voice);
