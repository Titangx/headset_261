...
#if 1	// titan for Y70 prompts
#define DEFAULT_NO_REPEAT_DELAY         0
#else
...
static bool uiPrompts_IsPromptMandatory(uint16 prompt_index)
{
    ...
    bool indication_mandatory = prompt_to_play.await_indication_completion;
    return indication_mandatory;
}
...
static void uiPrompts_PlayPrompt(uint16 prompt_index, rtime_t time_to_play, const ui_prompt_data_t *config)
{
    ...
    if (the_prompts.prompt_playback_enabled)
    {
        ...
        /* We should send the UI_MANDATORY_PROMPT_PLAYBACK_COMPLETED indication to the System State Manager when the prompt played is mandatory.
           By checking for uiPrompts_IsPromptMandatory, which checks await_indication_completion config bool
           for the prompt(intended for Power On basically), means, we can disable this behaviour for other apps, if needed.
        */

        if(uiPrompts_IsPromptMandatory(prompt_index))
        {
            MessageSendConditionally(SystemState_GetTransitionTask(), UI_MANDATORY_PROMPT_PLAYBACK_COMPLETED, NULL, ui_GetKymeraResourceLockAddress());
        }
...
#if 1	//titan for Y70 prompts
void uiPrompts_SchedulePromptPlay(uint16 prompt_index);
void uiPrompts_SchedulePromptPlay1(uint16 prompt_index);
void uiPrompts_SchedulePromptPlay(uint16 prompt_index)
{
...
static void uiPrompts_HandleShutdownRequest(void)
{
        ...
        (mandatory_prompt_is_playing || mandatory_power_off_prompt_reqd))
    {
        ...
        if (prompt_is_currently_playing &&
            ...
            (the_prompts.last_prompt_played_index != power_off_prompt_index))
        {
...
#if 1  //titan for fix charge issue when low battery state
the_prompts.uiprompts_init_finish_flag = TRUE;
#endif
    return TRUE;
}
...
#if 1  //titan for fix charge issue when low battery state
the_prompts.uiprompts_init_finish_flag = FALSE;
#endif
