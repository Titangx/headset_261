...
static cc_tx_status_t ccProtocol_CommsUartStatusToCcStatus(charger_comms_uart_tx_status status)
{
    ...
    if (status != CHARGER_COMMS_UART_TX_SUCCESS)
    {
        sts = CASECOMMS_TX_FAIL;
    }
...
static bool ccProtocol_GetEarbudToPoll(cc_dev_t* dev)
{
    ...
    for (int i=0; i<CASECOMMS_CID_MAX; i++)
    {
        ...
        DEBUG_LOG_V_VERBOSE("ccProtocol_GetEarbudToPoll Total after checking enum:cc_cid_t:%d l:%d r:%d", i, left_outstanding_response_total, right_outstanding_response_total);
    }
...
/* In case mode track the number of NAKs received, if it reaches 3 consider the
...
static void ccProtocol_CheckAckNakReset(cc_tx_status_t status)
{
    ...
    if (ccProtocol_TransRequiresReset())
    {
        ...
        else if (status == CASECOMMS_TX_FAIL)
        {
...
static void ccProtocol_HandleMessageChargercommsUartStatus(const MessageChargerCommsUartStatus* msg)
{
    ...
    DEBUG_LOG_VERBOSE("ccProtocol_HandleMessageChargercommsUartStatus enum:charger_comms_uart_tx_status:%d enum:charger_comms_uart_msg_type:%d enum:charger_comms_uart_address:%d cc_header:0x%x", msg->status, msg->header.type, msg->header.dest_address, msg->header.header);

    /* a non success status means the message was either not transmitted or failed to be
       acknowledged, so reduce the outstanding response count accordingly */
    if (   msg->status != CHARGER_COMMS_UART_TX_SUCCESS
        && td->mode == CASECOMMS_MODE_CASE
        && msg->header.dest_address != CHARGER_COMMS_UART_ADDRESS_BROADCAST)
    {
