...
static void dfu_StartPeerDfu(void)
{
    ...
    {
        DEBUG_LOG("dfu_StartPeerDfu upgrade scheme is not concurrent so not starting concurrent DFU");
        return;
    }
...
{
    DEBUG_LOG_INFO("dfu_HandleUpgradeOperationInd. Ops: %d", operation->ops);
    switch (operation->ops)
    {
        ...
        case upgrade_ops_handle_validation_state_change:
        {
#ifdef INCLUDE_DFU_PEER
            if(UPGRADE_PEER_IS_SUPPORTED && UpgradePeerWasPeerDfuInProgress())
            {
                UpgradeSmHandleValidationStateChange(TRUE);
            }
            else
#endif
            {
                UpgradeSmHandleValidationStateChange(FALSE);
            }
        }
            break;
        ...
        case upgrade_ops_handle_abort:
        {
            ...
            if(UPGRADE_PEER_IS_PRIMARY)
            {
                ...
                if (UpgradeGetHandoverDFUAbortState())
                {
                     ...
                     */
                    DfuPeer_ProcessHostMsg(UPGRADE_PEER_ERROR_WARN_RES, (uint8)UPGRADE_HANDOVER_ERROR_IND);
        ...
        case upgrade_ops_internal_handle_post_vldtn_msg_rcvd:
        {
            ...
            if(UPGRADE_PEER_IS_PRIMARY)
            {
                ...
                if(!UPGRADE_PEER_IS_STARTED)
                {
                     ...
                     */
                    bool start_peer_dfu = UpgradeSMHandleImageCopyStatusForPrim();
        ...
        case upgrade_ops_handle_post_vldtn_msg_rcvd:
        {
            ...
            if(UPGRADE_PEER_IS_PRIMARY)
            {
                if(!UPGRADE_PEER_IS_STARTED)
                {
                    ...
                    {
                        DEBUG_LOG("dfu_HandleUpgradeOperationInd: Setup Peer connection for DFU after Primary reset");
                    ...
                    DEBUG_LOG("dfu_HandleUpgradeOperationInd: Setup Peer connection for DFU");
                    if(start_peer_dfu && UpgradePeerStartDfu(UPGRADE_IMAGE_COPY_CHECK_REQUIRED)== FALSE)
                    {
        ...
        case upgrade_ops_save_peer_pskeys:
        {
            ...
            case upgrade_ops_set_validation_status:
            {
#ifdef INCLUDE_DFU_PEER
                /* For Serialized DFU: If handset link loss happens while transferring data to SEB, then
                 * after reconnecting with handset UPGRADE_VM_IMAGE_UPGRADE_COPY_SUCCESSFUL will get called 
                 * again for PEB which may result in primary reboot because is_validted flag is already
                 * set to 1.So, make sure is_validated is reset before calling IsValidatedToTrySwap().
                 */
                if(UpgradePeerGetDFUScheme() == upgrade_peer_scheme_serial)
                {
...
static void dfu_HandlePeerSigConnectInd(const PEER_SIG_CONNECTION_IND_T *ind)
{
    ...
    if (ind->status == peerSigStatusConnected)
    {
        ...
        /* Reset the peer DFU L2CAP disconnection reason to 0.
         * Though 0 represents l2cap_disconnect_successful, but DFU code only
         * checks for reason to be l2cap_disconnect_link_loss, so this reset is
         * fine to be able to indicate next disconnect reason correctly.
         */
        UpgradePeerStoreDisconReason(l2cap_disconnect_successful);
...
{
    uint16 actual_length = PsRetrieve(UPGRADE_CONTEXT_KEY, NULL, 0);
    if ((actual_length > 0) && (actual_length <= PSKEY_MAX_STORAGE_LENGTH))
    {
        uint16 key_cache[PSKEY_MAX_STORAGE_LENGTH];
        PsRetrieve(UPGRADE_CONTEXT_KEY, key_cache, actual_length);
        key_cache[APP_UPGRADE_CONTEXT_OFFSET] = (uint16) context;
        PsStore(UPGRADE_CONTEXT_KEY, key_cache, actual_length);
    }
}
