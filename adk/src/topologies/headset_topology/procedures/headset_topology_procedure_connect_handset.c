                                                  ...
                                                  Message goal_data)
{
    ...
    {
        HandsetService_ReconnectRequest(HeadsetTopProcConnectHandsetGetTask(), &td->handset_addr, chp->profiles);
...
void HeadsetTopology_ProcedureConnectHandsetCancel(procedure_cancel_cfm_func_t proc_cancel_cfm_fn)
{
    ...
    HandsetService_StopReconnect(HeadsetTopProcConnectHandsetGetTask(), &td->handset_addr);
}
...
static void headsetTopology_ProcConnectHandsetHandleHandleAvA2dpAudioDisconnected(void)
{
    ...
    if (td->audio_started)
    {
        HandsetService_ReconnectRequest(HeadsetTopProcConnectHandsetGetTask(), &td->handset_addr, td->profiles_requested);
    }
