...
rule_action_t ruleTwsTopPriPeerPairedOutCase(void)
{
    ...
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriPeerPairedOutCase, run as peer paired and out of case OR peer profile retention enabled");
    return rule_action_run;
}
...
rule_action_t ruleTwsTopPriConnectPeerProfiles(void)
{
    ...
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriConnectPeerProfiles run as out of case or peer profile retention enabled (profiles:x%x)", profiles);
...
rule_action_t ruleTwsTopPriReleasePeer(void)
{
    ...
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriReleasePeer ignore. Device not in case (normal) Or peer profile retention enabled");
    return rule_action_ignore;
}
...
rule_action_t ruleTwsTopPriSelectedPrimary(void)
{
    ...
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriSelectedPrimary, run as selected as Primary out of case, or in case with peer profile retention enabled but lid not closed");
    return rule_action_run;
}
...
rule_action_t ruleTwsTopPriSelectedActingPrimary(void)
{
    ...
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriSelectedActingPrimary, run as selected as Acting Primary out of case, or  in case with peer profile retention enabled but lid not closed");
    return rule_action_run;
}
...
rule_action_t ruleTwsTopPriNoRoleSelectedSecondary(void)
{
    ...
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriNoRoleSelectedSecondary, run as selected as Secondary out of case, or in the case with peer profile retention enabled but lid not closed");
    return rule_action_run;
}
...
rule_action_t ruleTwsTopPriPrimarySelectedSecondary(void)
{
    ...
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriPrimarySelectedSecondary, run as Primary out of case, or in case with peer profile retention enabled but lid not closed");
    return rule_action_run;
}
...
rule_action_t ruleTwsTopPriPeerLostFindRole(void)
{
    ...
    if (twsTopology_JustWentInCase() && (TwsTopology_PeerProfilesToRetain() == 0))
    {
...
rule_action_t ruleTwsTopPriEnableConnectableHandset(void)
{
    ...
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriEnableConnectableHandset, run as primary out of case or handset profile retention enabled and not connected to handset");
    return PRIMARY_RULE_ACTION_RUN_PARAM(enable_connectable);
}
...
rule_action_t ruleTwsTopPriEnableLeConnectableHandset(void)
{
    ...
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriEnableLeConnectableHandset, run as primary out of case or in case with handset profile retention enabled and not connected to handset");
...
static rule_action_t ruleTwsTopPriConnectHandset(rule_connect_reason_t reason)
{
    ...
    if (   BtDevice_GetWasConnected(&handset_addr)
        || (reason & (rule_connect_out_of_case | rule_connect_user | rule_connect_post_handover)))
    {
...
/*! Select the correct Le advertise parameters */
rule_action_t ruleTwsTopPriSelectLeAdvertiseParams(void)
{
    LE_ADV_PARAMS_SET_TYPE_T params_type;
    tws_topology_le_adv_params_set_type_t *type = &params_type.type;

    if (!BtDevice_IsPairedWithPeer())
    {
        TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriSelectLeAdvertiseParams, ignore as peer not paired");
        return rule_action_ignore;
    }

    /* If the handover is in progress then there is no need of LE advertisement */
    ...
    {
        TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriSelectLeAdvertiseParams, ignore as handover is in progress");
        return rule_action_ignore;
    }

    /* If the pairing is in progress, there will be need of fast LE advertisement
     * to expedite device discovery and pairing process */
    if (!appPairingIsIdle())
    {
        *type = LE_ADVERTISING_PARAMS_SET_TYPE_FAST;
    }
    /* If the peer is connected, select slow adverstiment.
     * However fast advertisement for configurable time period with fallback to slow adverstiment
     * can be selected if no handset devices are connected */
    else if (appDeviceIsPeerConnected())
    {
        if (appDeviceIsHandsetConnected())
        {
            *type = LE_ADVERTISING_PARAMS_SET_TYPE_SLOW;
        }
        else
        {
            *type = LE_ADVERTISING_PARAMS_SET_TYPE_FAST_FALLBACK;
        }
    }
    /* Always select fast advertisement when peer is not connected */
    else
    {
        *type = LE_ADVERTISING_PARAMS_SET_TYPE_FAST;
    }
    TWSTOP_PRIMARY_RULE_LOG("ruleTwsTopPriSelectLeAdvertiseParams, run with advertisement enum:LE_ADV_PARAMS_SET_TYPE_T:%d", *type);
    return PRIMARY_RULE_ACTION_RUN_PARAM(params_type);
}
