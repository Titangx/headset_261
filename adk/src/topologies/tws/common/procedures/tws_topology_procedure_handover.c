...
static void twsTopology_ProcHandoverReset(void)
{
    ...
    /*There might be a race condition of HandoverReset while TWSTOP_PRIMARY_GOAL_HANDOVER_START
     is in vm_queue. Therefore cancel the rule message or the app may PANIC when trying to handle it */
    MessageCancelFirst(TwsTopologyGetGoalTask(), TWSTOP_PRIMARY_GOAL_HANDOVER_START);
}
...
static void twsTopology_ProcHandoverStart(void)
{
    ...
    if(appDeviceIsHandsetConnected() && TwsTopologyGetTaskData()->handover_info.reason)
    {
...
static void twsTopology_ProcHandoverCancelOrFailCfmSend(void)
{
    ...
    if (td->handover_result == handover_failed)
    {
