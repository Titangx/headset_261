...
rule_action_t ruleTwsTopClassicPriNoRoleIdle(void)
{
    ...
    if (TwsTopology_IsDfuInCase() && appPhyStateGetState() == PHY_STATE_IN_CASE)
    {
        ...
        DEBUG_LOG("ruleTwsTopClassicPriNoRoleIdle, ignore as in-case DFU pending to retain links");
        return rule_action_ignore;
    }
    ...
    /* Check for peer profile retention is kept at end so that it does not cause
     * handover to get disallowed */
    if (TwsTopology_PeerProfilesToRetain() != 0)
    {
        DEBUG_LOG("ruleTwsTopClassicPriNoRoleIdle, ignore as peer profile retention enabled");
        return rule_action_ignore;
    }
...
rule_action_t ruleTwsTopClassicPriEnableConnectableHandset(void)
{
	...
	DEBUG_LOG("ruleTwsTopClassicPriEnableConnectableHandset, run as primary out of case or in case with profiles retained but not connected to handset");
...
rule_action_t ruleTwsTopClassicPriHandoverStart(void)
{
    ...
    {
        /* ToDo: Need to decide on the scope to clear it. */
        DEBUG_LOG("ruleTwsTopClassicPriHandoverStart, Ignored as in-case DFU is pending ");
        return rule_action_ignore;
    }
