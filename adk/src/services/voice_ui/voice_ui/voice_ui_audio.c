...
static bool VoiceUi_IsMicInUse(hfp_link_priority priority)
{
    hfp_call_state hfp_state;
    HfpLinkGetCallState(priority, &hfp_state);
    return (hfp_state != hfp_call_state_idle);
}

bool VoiceUi_IsHfpIsActive(void)
{
    bool mic_in_use = VoiceUi_IsMicInUse(hfp_primary_link);
    ...
    if (!mic_in_use)
    {
        ...
        mic_in_use = HfpProfile_IsScoActive();
