...
static FILE_INDEX amaAudio_FindLocalePromptFileIndex(const char *locale, ama_audio_prompt_t prompt)
{
    ...
    case ama_audio_prompt_unregistered:
        prompt_name = "_ama_unregistered.sbc";
        break;

    case ama_audio_prompt_not_connected:
        prompt_name = "_ama_not_connected.sbc";
        break;

    default:
        return FILE_NONE;
    }
    size_filename = strlen(locale) + strlen(prompt_name) + 1;
    char *filename = PanicUnlessMalloc(size_filename);
    strcpy(filename, locale);
    strcat(filename, prompt_name);
    file_index = FileFind(FILE_ROOT, filename, strlen(filename));
    free(filename);
    ...
    if (file_index == FILE_NONE)
    {
        ...
        if (strcmp(model, locale) != 0)
        {
            ...
            size_filename = strlen(model) + strlen(prompt_name) + 1;
            filename = PanicUnlessMalloc(size_filename);
...
inline static void amaAudio_SetCurrentLocaleFileIndex(void)
{
    ...
    PanicZero(current_locale.file_index = FileFind(FILE_ROOT, model, strlen(model)));
}
...
static void amaAudio_PlayPrompt(ama_audio_prompt_t prompt)
{
    ...
    if (AmaAudio_GetDeviceLocale(locale, AMA_LOCALE_STR_SIZE))
    {
        ...
        {
        case ama_audio_prompt_unregistered:
            file_index = amaAudio_FindLocalePromptFileIndex(locale, ama_audio_prompt_unregistered);
            break;
    ...
    if (file_index == FILE_NONE)
    {
        DEBUG_LOG_WARN("amaAudio_PlayPrompt: localised file not found");
        switch (prompt)
        {
        case ama_audio_prompt_unregistered:
            file_index = amaAudio_FindLocalePromptFileIndex(AMA_DEFAULT_LOCALE, ama_audio_prompt_unregistered);
            break;

        case ama_audio_prompt_not_connected:
            file_index = amaAudio_FindLocalePromptFileIndex(AMA_DEFAULT_LOCALE, ama_audio_prompt_not_connected);
            break;
        }
    }
    ...
    if (file_index != FILE_NONE)
    {
        ttp = rtime_add(SystemClockGetTimerTime(), AMA_AUDIO_SS_DELAY_US);
        appKymeraPromptPlay(file_index, PROMPT_FORMAT_SBC, 48000, ttp, FALSE, NULL, 0);
    }
    else
    {
        DEBUG_LOG_ERROR("amaAudio_PlayPrompt: default file not found");
        switch (prompt)
        {
        case ama_audio_prompt_unregistered:
