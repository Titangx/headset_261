...
/*! \brief Get the cached device from a state machine entry
    ...
    This helper function gets the cached handset device.
 ...
 */
static device_t handsetServiceSm_GetHandsetDeviceIfValid(handset_service_state_machine_t *sm)
{
...
static void handsetServiceSm_EnterConnectedBredr(handset_service_state_machine_t *sm)
{
    ...
    BtDevice_SetLastConnectedProfilesForDevice(handset_device, connected_profiles, TRUE);
...
{
    HS_LOG("handsetServiceSm_HandleInternalConnectAclComplete state:enum:handset_service_state_t:0x%x", sm->state);
...
{
    HS_LOG("handsetServiceSm_HandleProfileManagerConnectCfm enum:handset_service_state_t:0x%x enum:profile_manager_request_cfm_result_t:%d [%04x,%02x,%06lx]",
           sm->state,
           cfm->result,
           sm->handset_addr.nap,
           sm->handset_addr.uap,
...
{
    HS_LOG("handsetServiceSm_HandleProfileManagerDisconnectCfm enum:handset_service_state_t:0x%x enum:profile_manager_request_cfm_result_t:%d [%04x,%02x,%06lx]",
           sm->state, 
           cfm->result,
           sm->handset_addr.nap,
           sm->handset_addr.uap,
...
{
    HS_LOG("HandsetServiceSm_HandleProfileManagerConnectedInd device 0x%x enum:handset_service_state_t:0x%x profile 0x%x [%04x,%02x,%06lx]",
           ind->device,
           sm->state,
           ind->profile,
           sm->handset_addr.nap,
...
{
    HS_LOG("HandsetServiceSm_HandleProfileManagerDisconnectedInd device 0x%x enum:handset_service_state_t:0x%x profile 0x%x enum:profile_manager_disconnected_ind_reason_t:%d [%04x,%02x,%06lx]",
           ind->device,
           sm->state,
           ind->profile,
           ind->reason,
    ...
    const CON_MANAGER_TP_CONNECT_IND_T *ind)
{
    ...
    if(!sm->handset_device)
    {
        ...
        device = BtDevice_GetDeviceForBdAddr(&tpbdaddr.taddr.addr);
