...
typedef struct
{
    ...
    /*! bdaddr that have been requested. */
...
typedef struct
{
    ...
    /*! bdaddr tried to reconnect currently. Using this 
        in stop connect request */
    bdaddr bdaddr_connecting_now;
...
/*! \brief Connect to the previously connected  handset(s).
    ...
    \note #addr of the handset will be used just to compare with what Focus Device 
          (Focus_GetDeviceForUiInput()) returns. if both match then provided #profiles 
          is connected as part of connection only if #profiles are DFU related.
          Last connected profiles are used otherwise.
*/
