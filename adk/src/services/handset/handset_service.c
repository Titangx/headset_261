...
void HandsetService_StopReconnect(Task task, const bdaddr *addr)
{
    HS_LOG("HandsetService_StopReconnect task[0x%p] addr [%04x,%02x,%06lx] ",
           task, 
           addr->nap, 
           addr->uap, 
           addr->lap);

    HandsetServiceMultipointSm_StopReconnect(task, addr);
}
...
void HandsetService_ReconnectRequest(Task task, const bdaddr *addr, uint32 profiles)
{
    DEBUG_LOG_FN_ENTRY("HandsetService_ReconnectRequest");
    HandsetServiceMultipointSm_ReconnectRequest(task, addr, profiles);
}
