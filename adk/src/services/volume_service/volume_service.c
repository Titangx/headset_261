...
static bool volumeService_IsAudioSourceUiInput(ui_input_t ui_input)
{
    switch(ui_input)
    {
    case ui_input_audio_volume_up_start:
    case ui_input_audio_volume_down_start:
    case ui_input_audio_volume_up_stop:
    case ui_input_audio_volume_down_stop:
    case ui_input_audio_volume_up:
    case ui_input_audio_volume_down:
        return TRUE;
    default:
        return FALSE;
    }
...
static void volumeService_HandleUiInput(MessageId ui_input)
{
    ...
    if (volumeService_IsAudioSourceUiInput(ui_input))
    {
        handle_ui_input = Focus_GetAudioSourceForUiInput(ui_input, &audio_source);
        if (handle_ui_input)
        {
            step_size = VolumeUtils_GetStepSize(AudioSources_GetVolume(audio_source).config);
    ...
    {
        handle_ui_input = Focus_GetVoiceSourceForUiInput(ui_input, &voice_source);
        if (handle_ui_input)
        {
            step_size = VolumeUtils_GetStepSize(VoiceSources_GetVolume(voice_source).config);
    ...
    if (handle_ui_input)
    {
        ...
        case ui_input_audio_volume_up_stop:
        case ui_input_audio_volume_down_stop:
        case ui_input_voice_volume_stop:
            volumeService_StopVolumeRamp();
            break;

        case ui_input_audio_volume_up:
            VolumeService_ChangeAudioSourceVolume(audio_source, step_size);
            break;
        ...
        case ui_input_voice_volume_up:
            volumeService_ChangeVoiceVolume(voice_source, step_size);
            break;

        case ui_input_voice_volume_down:
            volumeService_ChangeVoiceVolume(voice_source, -step_size);
            break;
