...
static void peer_pair_le_handle_primary_service_discovery(const GATT_DISCOVER_PRIMARY_SERVICE_CFM_T* discovery)
{
    ...
    {
        DEBUG_LOG("peer_pair_le_handle_primary_service_discovery failure sts:%d", discovery->status);
...
static void peer_pair_le_handler(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        default:
            DEBUG_LOG("peer_pair_le_handler. Unhandled message MESSAGE:peer_pair_le_internal_message_t:0x%x", id);
            break;
    }
} 
