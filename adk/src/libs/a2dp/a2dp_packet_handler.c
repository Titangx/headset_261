...
static bool sinkInsertHeaderAndFlushData(Sink sink, uint16 amount_to_flush, const uint8 *header, uint16 hdrlen)
{
    ...
    PRINT(("sinkInsertHeaderAndFlushData prev_sink_claimed %d\n", prev_sink_claimed));
    ...
    if (prev_sink_claimed != 0xFFFF)
    {
        ...
        PRINT(("sinkInsertHeaderAndFlushData flushing amount_to_flush %d hdrlen %d result %d\n", amount_to_flush, hdrlen, flush_result));
    ...
    PRINT(("sinkInsertHeaderAndFlushData not flushing\n"));
...
static bool sendPacket(remote_device *device, uint8 signalling_header, uint8 signal_id, uint16 payload_size)
{
    ...
    PRINT(("sendPacket device=%p signalling_header=%X signal_id=%u mtu=%u payload_size=%u  <", (void*)device, signalling_header, signal_id, signalling->connection.active.bitfields.mtu, payload_size));
    ...
    {   /* Enough space in MTU to fit packet and two byte header */
        PRINT(("sendPacket Enough space in MTU to fit packet and two byte header\n"));
    ...
    {   /* Packet won't fit in MTU and needs to be fragmented */
        /* Start Packet */
        PRINT(("sendPacket Packet won't fit in MTU and needs to be fragmented\n"));
