...
void a2dpHandleSignallingUseLargeMTU(const A2DP_INTERNAL_SIGNALLING_MTU_REQ_T *req)
{
    ...
    PRINT(("a2dpHandleSignallingUseLargeMTU device=%p, enable=%d", (void*)device, req->use_large_mtu));
...
void a2dpHandleL2capConnectInd(const CL_L2CAP_CONNECT_IND_T *ind)
{
    ...
    if(ind->psm == AVDTP_PSM)
    {
        ...
        {   /* We are connecting/have connected to this device */
            ...
            PRINT((" signalling state=%u, large mtu=%d\n",signalling->status.connection_state, device->bitfields.use_large_mtu));
