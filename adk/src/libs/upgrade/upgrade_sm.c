...
bool upgradeSm_HandleAborting(MessageId id, Message message)
{
    ...
    case UPGRADE_HOST_ERRORWARN_RES:
        ...
        {
            DEBUG_LOG("upgradeSm_HandleAborting Handover DFU abort: ERRORWARN_RES skipped");
             ...
             */
            DEBUG_LOG("upgradeSm_HandleAborting ImageUpgradeAbortCommand ret:%d", ImageUpgradeAbortCommand());
    ...
    case UPGRADE_HOST_ABORT_REQ:
        ...
        if (UpgradeGetHandoverDFUAbortState() &&
                !UpgradeIsHandoverDFUAbortComplete())
        {
            DEBUG_LOG("upgradeSm_HandleAborting Handover DFU abort: ABORT_REQ skipped");
    ...
    case UPGRADE_INTERNAL_CONTINUE:
        {
            if(ctx->vctx != NULL)
            {
                DEBUG_LOG_VERBOSE("upgradeSm_HandleDataHashChecking: Already in progress");
            }
            else
            {
    ...
    case UPGRADE_INTERNAL_CONTINUE:
        {
            ...
            {
                UpgradeSendUpgradeOpsStatus(UpgradeGetAppTask(), upgrade_ops_handle_validation_state_change, NO_ACTION);
            }
        }
        break;
    ...
    if (!handled)
    {
        ...
        case UPGRADE_HOST_ABORT_REQ:                        
            ...
            if (UpgradeGetHandoverDFUAbortState() &&
                    !UpgradeIsHandoverDFUAbortComplete())
            {
                DEBUG_LOG("upgradeSm_DefaultHandler Handover DFU abort: ABORT_REQ skipped");
                UpgradeSetHandoverDFUAbortState(handover_upgrade_abort_abort_req_recvd);
                /*
                 * Reset the the upgrade library state as erase is bypassed for
                 * Handover triggered DFU abort.
       ...
       case UPGRADE_HOST_ERRORWARN_RES:
            {
                ...
                if (err_res->errorCode == UPGRADE_HOST_ERROR_HANDOVER_DFU_ABORT)
                {
                     ...
                     * local & peer if supported), required for seemless 
                     * DFU especially after a Handover completion.
                     */
                    UpgradeSavePSKeys();
                    UpgradeSendUpgradeOpsStatus(UpgradeGetAppTask(), upgrade_ops_save_peer_pskeys, NO_ACTION);
                     ...
                     */
                    DEBUG_LOG("upgradeSm_DefaultHandler ImageUpgradeAbortCommand ret:%d", ImageUpgradeAbortCommand());
#endif  /* MESSAGE_IMAGE_UPGRADE_COPY_STATUS */
                    /*
...
#ifdef HANDOVER_DFU_ABORT_WITHOUT_ERASE
    if(error_code == UPGRADE_HOST_ERROR_HANDOVER_DFU_ABORT)
    {
        /*
         ...
         */
        DEBUG_LOG("UpgradeFatalError, Only error report on Handover DFU abort.");
    }
    else
#endif
    {
...
*/
void UpgradeSmHandleValidationStateChange(bool was_peer_dfu_in_progress)
{
    /* If the primary device is reset during prim to sec data (for serialized DFU),
     * then by resume point data, we will land here. Since image copy is 
     * already completed, don't perform again. Just pass the COPY_SUCCESSFUL id
     * to UpgradeSMHandleValidated.
     * Also set the state as UPGRADE_STATE_VALIDATED, since it was the state before reset.
     */
    if(was_peer_dfu_in_progress)
    {
        UpgradeCtxGet()->isImgUpgradeCopyDone = TRUE;
        UpgradeSMHandleValidated(UPGRADE_VM_IMAGE_UPGRADE_COPY_SUCCESSFUL, NULL);
        UpgradeSMSetState(UPGRADE_STATE_VALIDATED);
...
void UpgradeSmHandleInProgressInd(bool is_peer_not_connected, uint32 peer_poll_interval, uint8 action)
{
    ...
    msg = (UPGRADE_HOST_IN_PROGRESS_RES_T *)PanicUnlessMalloc(sizeof(UPGRADE_HOST_IN_PROGRESS_RES_T));
    msg->action = action;
