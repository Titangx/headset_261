...
/**
 * \brief  Calculate time taken in frames for gentle mute
 *
 * \param timer_param  gentle mute time in seconds
 *
 * \return  uint16 value indicating number of frames
 */
    ...
    for (i=0; i<num_controls; i++)
    {
        ...
        else if (ctrl_id == AANC_CONSTANT_FILTER_CONFIG_CTRL)
        {
            ...
            if (ctrl_src == CPS_SOURCE_HOST)
            {
                ...
                L4_DBG_MSG1("AANC filter configuration override: %d",
                            p_params->configuration);
